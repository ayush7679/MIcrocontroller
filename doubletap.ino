...
// ── Cardholder names ─────────────────────────────────────────
// Keep these reasonably short — they get printed on a 16-char LCD line.
const char* normalName  = "Balen Shah";
const char* specialName = "Mahabir Pun";
...
void loop() {
  ...
  if (matchUID(rfid.uid.uidByte, normalUID)) {
    lcdLine(0, normalName);
    lcdLine(1, "Processing...   ");
    // Python decides IN vs OUT based on its own state.
    // We just report WHICH card tapped, plus the name for display.
    // Format → NORMAL:Balen Shah
    Serial.print("NORMAL:");
    Serial.println(normalName);

  } else if (matchUID(rfid.uid.uidByte, specialUID)) {
    lcdLine(0, specialName);
    lcdLine(1, "Processing...   ");
    // Format → SPECIAL:Mahabir Pun
    Serial.print("SPECIAL:");
    Serial.println(specialName);
